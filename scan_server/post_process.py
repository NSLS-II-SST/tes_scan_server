...
def get_scans_and_calibrations(beamtime_dir):
    ...
    print("get_scans_and_calibrations globbing: "+os.path.join(beamtime_dir, "scan*.json"))
    scan_filenames = glob.glob(os.path.join(beamtime_dir, "scan*.json"))
    cal_filenames = glob.glob(os.path.join(beamtime_dir, "calibration*.json"))
    scans = [scan_server.DataScan.from_file(f) for f in sorted(scan_filenames)]
    calibrations = [scan_server.CalibrationScan.from_file(f) for f in sorted(cal_filenames)]
    scans = OrderedDict((scan.scan_num ,scan) for scan in scans)
    calibrations = OrderedDict((cal.scan_num, cal) for cal in calibrations)
...
def process(scan, calibrations, drift_correction_plan, bin_edges_ev, output_dir, max_channels=10000):
    ...
    cal_number = scan.cal_number
    cal = calibrations[cal_number]
    if drift_correction_plan == "basic":
        drift_correct_states = [f"CAL{cal_number}", f"SCAN{scan.scan_num}"]
    ...
    routine(cal.scan_num, data, attr=attr, calibratedName="energy")
