...
def get_method_from_data(data):
    ...
    if "method" not in d.keys():
        return None, None, None, _id, f"method key does not exist"
    method_name = d["method"]
    args = d.get('params', [])
    kwargs = d.get('kwargs', {})
    return method_name, args, kwargs, _id, None
...
class RPCDispatch(QObject):
    ...
    def call_method(self, method_name, args=[], kwargs={}, no_traceback_error_types=[]):
        ...
        try:
            ...
            return None, f"Calling Exception: method={method_name}: {e}"

    def handle_one_message(self, sock, data, verbose, no_traceback_error_types):
